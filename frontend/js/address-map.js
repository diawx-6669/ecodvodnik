...
function amInit() {
  ...
  // троттлинг публичного сервера для сторонних сайтов: одни запросы проходят,
  // другие получают 429/обрыв, и так без остановки, независимо от конкретного
  // провайдера (даже у ArcGIS/Esri такое иногда бывает под нагрузкой). Раньше
  // карта переключалась на резервный слой только если НИ ОДНА плитка не
  // загрузилась вообще — а при частичном сбое (самый частый случай) не
  // переключалась никогда, и часть карты навсегда оставалась серыми клетками.
  // Теперь слой считает ДОЛЮ ошибок среди всех запросов (а не факт наличия
  // хоть одного успеха) и проверяет её сразу, как только текущая партия
  // тайлов на экране "устаканилась" (событие 'load'), а не только раз в
  // несколько панорамирований — поэтому дыры на самом первом виде карты
  // теперь тоже ловятся и провайдер меняется быстро.
  // Раньше здесь были только "уличные"/векторные стили (Esri World Street
  // Map, OSM, Wikimedia, HOT) — они рисуют дороги и подписи, но НЕ реальную
  // фотографию земли. Найти на такой карте конкретный дом почти невозможно:
  // видно только линии улиц без крыш и построек. Esri World Imagery — это
  // бесплатные (без ключа API) спутниковые снимки с тем же протоколом
  // тайлов, что и у остальных провайдеров — на них видно реальные крыши
  // домов, и выбирать точку по факту становится реально можно. Ставим его
  // первым; уличные стили остаются ниже как резерв, если спутник вдруг не
  // загрузится (например, недоступен домен) — хоть с дорогами, но карта.
  ...
  // Если карта хотя бы раз успешно показала настоящие тайлы, дальше её
  // НЕЛЬЗЯ полностью сносить в офлайн-режим — иначе получается, что карта
  // работала, пользователь приблизил её, чтобы точнее выбрать дом, а на
  // этом уровне зума часть тайлов у всех провайдеров подряд ответила
  // ошибкой (обычно кратковременная перегрузка публичных серверов именно
  // на глубоком зуме) — и вместо "потерпи секунду" человек получал полную
  // потерю карты (замену на серую офлайн-сетку), хотя мгновением раньше
  // всё прекрасно грузилось. Once true, аварийный сценарий ниже больше не
  // трогает сам объект map и не двигает вид — только пробует провайдеров
  // заново.
  ...
  function amStartProvider(index) {
    if (index >= AM_TILE_PROVIDERS.length) {
      if (everSucceeded) {
        // Карта уже доказала, что умеет грузиться нормально — проблема
        // локальна для текущего вида (обычно глубокий зум в месте с редким
        // покрытием тайлов или временная перегрузка серверов), а не с картой
        // в целом. Один раз мягко откатываем зум на пару уровней назад
        // (там тайлы почти наверняка есть) и по новой пробуем провайдеров
        // с начала, вместо того чтобы сносить рабочий Leaflet-объект.
        if (!rescueAttempted) {
          rescueAttempted = true;
          // Раньше здесь ещё и принудительно отдаляли вид (map.setZoom) —
          // но именно это мешало людям приближаться к своему дому: стоило
          // подойти близко к максимальному зуму, где какие-то тайлы у всех
          // провайдеров подряд на мгновение спотыкались, как карту тут же
          // само отдаляло обратно, будто откатывая ручной зум пользователя.
          // Оставляем вид ровно там, где его поставил человек, и просто
          // по новой пробуем провайдеров с начала.
          amSetStatus('Часть снимков карты на этом участке сейчас не загружается, пробуем другой источник снимков — вид оставлен как есть.', 'ok');
          amStartProvider(0);
          // Через минуту разрешаем повторный мягкий рестарт провайдеров,
          // если пользователь уедет в другое место и там снова возникнет
          // похожая ситуация — без этого один-единственный сбой в начале
          // сессии "тратил" защиту навсегда.
          setTimeout(() => { rescueAttempted = false; }, 60000);
          return;
        }
        // Второй раз подряд — не долбим бесконечно тем же откатом.
        // Оставляем карту как есть на последнем провайдере: часть тайлов
        // может не прогрузиться, но клики, метка и выбор дома продолжают
        // работать как обычно — это лучше, чем полная потеря карты.
        amSetStatus('Часть снимков карты на этом участке сейчас не загружается, но карта и выбор точки работают в обычном режиме.', 'ok');
        return;
      }
      // Карта ещё ни разу не показала ни одного живого тайла — вероятно,
      // сеть/блокировщик режет вообще все внешние тайлы. Переходим на
      // локальный офлайн-режим, чтобы точку на объекте всё равно можно
      // было выбрать.
      map.remove();
      amState.map = null;
      amRenderOfflineMap(mapEl);
