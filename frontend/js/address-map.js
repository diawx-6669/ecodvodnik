...
function amInit() {
  ...
  // трогает сам объект map — только откатывает зум и переключает провайдер.
    ...
    if (index >= AM_TILE_PROVIDERS.length) {
      if (everSucceeded) {
        ...
        if (!rescueAttempted) {
          ...
          const targetZoom = Math.max(AM_DEFAULT_VIEW.zoom, map.getZoom() - 2);
          if (targetZoom < map.getZoom()) map.setZoom(targetZoom);
          amSetStatus('На этом уровне приближения часть снимков карты сейчас не грузится — немного отдалил вид. Точку по-прежнему можно выбрать и уточнить кликом.', 'ok');
          amStartProvider(0);
          // Через минуту разрешаем повторный мягкий откат, если пользователь
          // уедет в другое место и там снова возникнет похожая ситуация —
          // без этого один-единственный сбой в начале сессии "тратил"
          // защиту навсегда и все последующие сбои уже просто молча
          // терпелись без попытки откатить зум.
